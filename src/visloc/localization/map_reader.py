...
from visloc.keypoint_pipeline.base import CombinedKeyPointAlgorithm
from visloc.keypoint_pipeline.typing import ImageKeyPoints
from visloc.localization.base import BaseMapReader
...
class SatelliteMapReader(BaseMapReader):
    ...
    def __init__(
        ...
        self._geo_metadata: Optional[pd.DataFrame] = None
...
class TileSatelliteMapReader(BaseMapReader):
    ...
    def __init__(
        ...
        cv2_read_mode: int = cv2.IMREAD_COLOR,
         ...
         # Use None for resize_size in super().__init__ for TileReader
        super().__init__(db_path, logger, None, cv2_read_mode)
        self.zoom_level = zoom_level
        self.flight_zone = flight_zone
        # Initialize specific attributes
        self._image_db: List[TileImage] = list()
        self._mosaic: Optional[Mosaic] = None # MosaicSatelliteMapReader uses this
...
class MosaicSatelliteMapReader(TileSatelliteMapReader):
    ...
    def __init__(
        ...
        self._mosaic: Optional[Mosaic] = None # Initialize mosaic attribute
