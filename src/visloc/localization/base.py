...
from visloc.tms.data_structures import DroneImage, GeoSatelliteImage
...
class BaseMapReader(ABC):
    ...
    def extract_features(
        ...
        ----------
        image : np.ndarray
            Image array to extract features from (algoritma ne bekliyorsa o formatta olmalı)
        algorithm : CombinedKeyPointAlgorithm
            Key point detection and description algorithm
        """
        # Bu yardımcı fonksiyon sadece algoritmayı çalıştırır, dönüşüm yapmaz
    ...
    def extract_features_from_image(
        ...
        """Extract features from an image in the database using the given algorithm.
           Handles grayscale conversion if the algorithm is SuperPoint.
        """
        img_item = self[image_name]
        if img_item.image is None:
             self.logger.warning(f"Image {image_name} not loaded, cannot extract features.")
             return

        image_to_process = img_item.image.copy() # Orijinali değiştirmemek için kopyala

        # --- BAŞLANGIÇ: DÜZELTME ---
        # Algoritma SuperPoint ise ve görüntü renkli ise gri tonlamaya çevir
        if isinstance(algorithm, SuperPointAlgorithm):
            if len(image_to_process.shape) == 3 and image_to_process.shape[2] == 3:
                 self.logger.debug(f"Converting image {image_name} to grayscale for SuperPoint.")
                 image_to_process = cv2.cvtColor(image_to_process, cv2.COLOR_BGR2GRAY)
            elif len(image_to_process.shape) != 2:
                 # Beklenmedik durum (örn. 4 kanal), hata ver veya logla
                 self.logger.error(f"Image {image_name} has unexpected shape {image_to_process.shape} for SuperPoint. Skipping feature extraction.")
                 return
        # --- BİTİŞ: DÜZELTME ---

        # Dönüştürülmüş veya orijinal görüntüyü kullanarak özellikleri çıkar
        try:
            kp: ImageKeyPoints = self.extract_features(image_to_process, algorithm)
            img_item.key_points = kp  # Sonucu orijinal öğeye ata
        except Exception as e:
            self.logger.error(f"Error extracting features for {image_name} with {algorithm.__class__.__name__}: {e}")
            # Hata durumunda key_points'i None olarak bırakabilir veya işaretleyebilirsiniz.
            img_item.key_points = None
    ...
    def describe_db_images(self, algorithm: CombinedKeyPointAlgorithm) -> None:
        ...
        # Artık _is_loaded kontrolüne gerek yok, setup_db içinde yükleniyor
        # if not self._is_loaded:
        #     raise ValueError("Images are not loaded, call load_images() first") # Bu kontrol kaldırılabilir

        self.logger.info(
            f"Describing images in the database using {algorithm.__class__.__name__}"
        )
        # _image_db içindeki her öğe için extract_features_from_image çağır
        for img_info in tqdm(self._image_db, desc=f"Describing DB with {algorithm.__class__.__name__}"):
            self.extract_features_from_image(img_info.name, algorithm)

        self._is_described = True # Bu flag hangi algoritmayla describe edildiğini göstermiyor, belki kaldırılmalı veya iyileştirilmeli
